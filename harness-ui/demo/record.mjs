...
const OUT = new URL('./lighthouse-reviewer-ui-demo.mp4', import.meta.url);
const outPath = decodeURIComponent(OUT.pathname.replace(/^\/([A-Za-z]:)/, '$1'));
if (existsSync(outPath)) rmSync(outPath);
const chrome = spawn(CHROME, [
  `--user-data-dir=${join(tmpdir(), 'lighthouse-reviewer-chrome-profile')}`,
  '--no-first-run', '--no-default-browser-check',
  '--window-size=1280,800', '--window-position=40,40',
  '--auto-select-tab-capture-source-by-title=Lighthouse reviewer recorder',
  'http://localhost:8798/record',
...
const timer = setInterval(() => {
  if (existsSync(outPath) && statSync(outPath).size > 1_000_000) {
    clearInterval(timer);
    console.log(`wrote ${outPath} (${statSync(outPath).size} bytes). Now run: node demo/fix-mp4-duration.mjs demo/lighthouse-reviewer-ui-demo.mp4 196`);
